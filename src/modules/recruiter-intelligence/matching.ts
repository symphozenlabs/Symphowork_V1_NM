...
export function matchCandidateToJob(candidate: CandidateProfile, requirement: JobRequirement, weights: Record<string, number> = {}) : MatchResult {
  ...
  const required = dimensions.filter((d) => d.dimension === "required_skills" || d.dimension === "relevant_experience"); const matchedRequired = required.filter((d) => d.result === "matched").length; const known = dimensions.filter((d) => d.result !== "unknown" && d.result !== "not_applicable"); const normalizedIndicator = known.length ? Math.round((dimensions.filter((d) => d.result === "matched").length / known.length) * 100) : 0;
